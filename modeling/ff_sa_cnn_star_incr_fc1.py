...
class net_one_neuron_ff_sa_cnn_star_incr_fc1(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        self.fcl_center = nn.Linear(30, 1)
        self.fcl_surr = nn.Linear((9*9 - 1) * 30, 1)
        self.fcl_comb = nn.Linear(2,1)
        ...
        for param in self.b_cpbs.parameters():
            param.requires_grad = False
        for param in self.fcl_center.parameters():
            param.requires_grad = False
    ...
    def forward(self, x):
        ...
        x1 = x.reshape(-1,30,81)
        x1 = x1[:,:,40]
        x1 = self.flatten(x1)
        x1 = self.fcl_center(x1)

        x2 = x.reshape(-1,30,81)
        x2 = x2[:, :, torch.cat((torch.arange(40), torch.arange(41, 81)))]
        x2 = self.flatten(x2)
        x2 = self.fcl_surr(x2)
...
class seperate_core_model_ff_sa_cnn_star_incr_fc1(nn.Module):
    def __init__(self,num_neurons):
        super().__init__()
        self.models = nn.ModuleList([net_one_neuron_ff_sa_cnn_star_incr_fc1() for i in range(num_neurons)])
...
def model_ff_sa_cnn_star_incr_fc1(num_neurons):
    return seperate_core_model_ff_sa_cnn_star_incr_fc1(num_neurons=num_neurons)

def model_ff_sa_cnn_star_incr_fc1_one_neuron():
	 return net_one_neuron_ff_sa_cnn_star_incr_fc1()