...
class net_one_neuron_rf_sa_cnn(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        self.attention = self_attention(in_dim=30,q_dim=5,k_dim=5,v_dim=5,v_mapping=False)
...
class seperate_core_model_rf_sa_cnn(nn.Module):
    def __init__(self,num_neurons):
        super().__init__()
        self.models = nn.ModuleList([net_one_neuron_rf_sa_cnn() for i in range(num_neurons)])
...
def model_rf_sa_cnn(num_neurons):
    return seperate_core_model_rf_sa_cnn(num_neurons=num_neurons)

def model_rf_sa_cnn_one_neuron():
	 return net_one_neuron_rf_sa_cnn()